...
template <class In>
void Str2::insert(const_iterator pos, In begin, In end)
{
	//Get the total amount of elements being inserted
	size_t insertLength = end - begin;
	//Get the element index we are inserting behind
	Str2::size_type index = pos - chars;
	//Grow memory to fit the newly inserted items
	grow(insertLength);
	//Update the position pointer 
	pos = chars + index;
	//Move all later elements down by length amount
	const_iterator s = pos;
	const_iterator e = chars + length;
	const_iterator dest = pos + insertLength;
