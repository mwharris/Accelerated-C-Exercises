...
public:
	...
	explicit Vec(iterator start, iterator end)
	{
		create(start, end);
	}
	...
	//Erase all elements in a range: [start, end)
	iterator erase(iterator start, iterator end)
	{
		//Cap our deletion to the end of the vector
		if (end > avail)
		{
			end = avail;
		}
		//Delete every element between start and end
		iterator it = start; 
		while (it != end)
		{
			alloc.destroy(it++);
		}
		//Move any elements back if needed
		if (end != avail)
		{
		...
		//If end = avail then all that needs to be done is to reset avail
		else
		{
			avail = start;
		}
