...
public:
	...
	*/
	...
	template <class In> void assign(In start, In end);
...
template <class T>
template <class In> 
void Vec<T>::assign(In start, In end)
{
	//Clear the contents of the vector
	this->clear();
	//Determine the amount of elements in the range
	size_t length = end - start;
	//Determine if memory needs to be added
	while (avail + length - 1 >= limit) {
		grow();
	}
	//Fill the memory with data
	avail = std::uninitialized_copy(start, end, data);
}
